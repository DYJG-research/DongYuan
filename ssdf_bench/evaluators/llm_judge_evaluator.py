...
from benchmark_core.model_interface import APIModelInterface
...
class LLMJudgeEvaluator:
    ...
    def __init__(self, api_host: str, api_port: int, model_name: str = "Qwen3-32B", api_key: Optional[str] = None):
        ...
        # If no api_key is provided, use a placeholder; some local OpenAI-compatible
        # backends ignore the api_key field entirely.
    ...
    def prompt_loader(self, filename) -> str:
        ...
        Resolves relative to the project root (parent of the ssdf_bench directory).
        """
        # Resolve the project root: ssdf_bench/evaluators -> ssdf_bench -> project root
        project_root = Path(__file__).resolve().parents[1]
        prompt_path = project_root / "evaluation_prompts" / filename

        if not prompt_path.exists():
            # Fallback: try a prompts/ directory adjacent to evaluators/
            prompt_path = project_root.parent / "prompts" / filename

        if not prompt_path.exists():
            raise FileNotFoundError(
                f"Prompt file not found: {filename}. "
                f"Searched in {project_root / 'evaluation_prompts'} "
                f"and {project_root.parent / 'prompts'}."
            )
